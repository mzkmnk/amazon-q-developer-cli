...
///
///
/// [`SendMessageOutput`](crate::operation::send_message::SendMessageOutput) contains an event
/// stream field as well as one or more non-event stream fields. Due to its current implementation,
/// the non-event stream fields are not fully deserialized until the [`send`](Self::send) method
/// completes. As a result, accessing these fields of the operation output struct within an
/// interceptor may return uninitialized values.
